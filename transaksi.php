...
$summaryResult = $koneksi->query(
    'SELECT COUNT(*) AS jumlah_transaksi, COALESCE(SUM(total_harga), 0) AS total_pendapatan FROM transaksi'
);
...
<main class="mx-auto max-w-7xl space-y-6 px-4 py-6">
    <section class="space-y-2">
        <h2 class="text-2xl font-extrabold tracking-tight text-slate-900">Riwayat Penjualan</h2>
                    ...
                    <?php if ($transaksiResult && $transaksiResult->num_rows > 0): ?>
                        <?php while ($transaksi = $transaksiResult->fetch_assoc()): ?>
                            ...
                            <tr class="hover:bg-slate-50/80">
                                <td class="px-4 py-4 font-semibold text-slate-900">#<?= h($transaksi['id_transaksi']) ?></td>
                                ...
                                <td class="px-4 py-4 text-right">
                                    <button type="button" class="inline-flex items-center justify-center rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-indigo-700" onclick="showDetail(<?= (int) $transaksi['id_transaksi'] ?>)">Lihat Detail</button>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="6" class="px-4 py-10 text-center text-sm text-slate-500">Belum ada riwayat transaksi.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
...
<div id="detailModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-950/50 px-4 py-6">
    <div class="w-full max-w-3xl overflow-hidden rounded-2xl bg-white shadow-2xl shadow-slate-900/20" onclick="event.stopPropagation()">
        <div class="flex items-start justify-between border-b border-slate-200 px-6 py-5">
            ...
            <button type="button" onclick="closeModal()" class="rounded-xl p-2 text-slate-500 transition hover:bg-slate-100 hover:text-slate-700" aria-label="Tutup modal">
                <i data-lucide="x" class="h-5 w-5">x</i>
            </button>
        </div>
        ...
        <div class="space-y-6 p-6">
            <div class="grid gap-4 md:grid-cols-3">
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    <div class="text-xs uppercase tracking-wide text-slate-500">Uang Bayar / Kembalian</div>
